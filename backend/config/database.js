...
// testConnection(); // testa a conexão ao iniciar o aplicativo (desabilitado - usando Prisma)

// Funções CRUD (Create, Read, Update, Delete) para interagir com o banco de dados.
async function Read(table, where = null) {
  // lê dados de uma tabela, pode ser filtrado ou não
  ...
  try {
    ...
    if (where) {
      // se houver um filtro, adiciona a cláusula WHERE
...
async function Update(table, data, where) {
  //função para arualizar um registro e, uma tabela
  const connection = await getConnection();

  try {
    const set = Object.keys(data)
      .map((key) => `${key} = ?`)
      .join(", "); // ele substitui as chaves do objeto data por placeholders para a query SQL (ex: "name = joão,age = 30) => "name = ?, age = ?")
...
async function Delete(table, where) {
  // funcção para deletar um registro de uma tabela
  const connection = await getConnection();
...
async function comparePassword(password, hash) {
  // função para comparar uma senha com um hash armazenado no banco de dados
  try {
...
async function FindOne(table, where, values) {
  // função para pesquisar se um objeto já existe por parametros
  const connection = await getConnection();
...
export {
  // exporta as funções criadas a cima
  getConnection,
  Read,
  Create,
  Update,
