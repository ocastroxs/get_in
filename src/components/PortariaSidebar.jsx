...
const PORTARIA_ITEMS = [
  ...
  { href: '/portaria/pendencias', icon: BadgeAlert, label: 'Pendências' },
  { href: '/portaria/aprovacoes', icon: BadgeCheck, label: 'Aprovações' },
  { href: '/portaria/historico', icon: Album, label: 'Histórico' },
];
  ...
  return (
    <>
      ...
      <div
        ...
        aria-hidden="true"
      />
      <MobileNav items={PORTARIA_ITEMS} isActive={isActive} />
    </>
  );
}
...
function MobileNav({ items, isActive }) {
  return (
    <nav
      ...
      aria-label="Navegação da portaria"
    >
      {items.map(({ href, icon: Icon, label }) => {
        ...
        return (
          <Link
            key={href}
            href={href}
            className={`flex min-w-[82px] flex-col items-center justify-center gap-1 rounded-xl px-2 py-2 text-[10px] font-semibold transition-colors ${
              active
